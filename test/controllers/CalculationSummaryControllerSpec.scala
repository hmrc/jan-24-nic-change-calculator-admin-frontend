  ...
  "onPageLoad" - {
    ...
    "must return OK and the correct view" - {
      ...
      "for a get with a `from` query string parameter" in {
        ...
        val summaryData = CalculationSummaryData(Some(from), None, 1, 2, 3, 4, 5, 6, 7)
      ...
      "for a get with a `to` query string parameter" in {
        ...
        val summaryData = CalculationSummaryData(None, Some(to), 1, 2, 3, 4, 5, 6, 7)
      ...
      "for a get with `from` and `to` query string parameters" in {
        ...
        val summaryData = CalculationSummaryData(Some(from), Some(to), 1, 2, 3, 4, 5, 6, 7)
